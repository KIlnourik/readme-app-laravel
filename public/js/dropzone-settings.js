...
function dropzoneInit() {
    const registerZone = document.querySelector(".registration__file-zone");
    const addPostPhotoZone = document.querySelector(".adding-post__file-zone--photo");
    const addPostVideoZone = document.querySelector(".adding-post__file-zone--video");
    const fileInputs = document.querySelectorAll('input[type="file"]');

    if (fileInputs) {
        fileInputs.forEach(input => input.addEventListener("click", function (e) {
            e.preventDefault();
        }));
    }

    if (registerZone) {
        new Dropzone(".registration__file-zone", {
            url: '#',
            maxFiles: 1,
            init: function () {
                this.on("addedfile", function () {
                    console.log(this.files);
                    this.files[1] != null && this.removeFile(this.files[0]);
                });
            },
    ...
    if (addPostVideoZone) {
        new Dropzone(".adding-post__file-zone--video", {
            ...
            autoProcessQueue: false,
            previewTemplate:
                '<div class="dz-preview dz-file-preview"><div class="adding-post__video-wrapper form__file-wrapper form__file-wrapper--video"> <img class="form__image" src="" alt="" data-dz-thumbnail> </div> <div class="adding-post__file-data form__file-data"> <span class="adding-post__file-name form__file-name dz-filename" data-dz-name></span> <button class="adding-post__delete-button form__delete-button button" type="button" data-dz-remove> <span>Удалить</span> <svg class="adding-post__delete-icon form__delete-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18" width="12" height="12"><path d="M18 1.3L16.7 0 9 7.7 1.3 0 0 1.3 7.7 9 0 16.7 1.3 18 9 10.3l7.7 7.7 1.3-1.3L10.3 9z"/></svg> </button> </div></div>',
        });
    }
}
